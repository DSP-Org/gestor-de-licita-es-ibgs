      ...
      {loading ? (
        ...
        <div className="text-center py-16 bg-card border rounded-2xl p-8 space-y-3">
          ...
          <p className="text-sm text-muted-foreground max-w-md mx-auto">
            {busca || filtroStatus !== "todos" || listaSelecionada
              ? "Tente ajustar os filtros ou a pasta selecionada."
              : "Favorite licitações no painel inicial para acompanhá-las em seu funil."}
          </p>
        </div>
      ) : modo === "kanban" ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-4 items-start overflow-x-auto pb-4">
          {ETAPAS_FUNIL.map((etapa) => {
            const itensEtapa = filtradas.filter((l) => (l.status || "interessado") === etapa.id);
            const totalEtapaValor = itensEtapa.reduce((s, l) => s + (Number(l.valor) || 0), 0);

            return (
              <div key={etapa.id} className={`rounded-xl border ${etapa.color} p-3 flex flex-col gap-3 min-w-[240px]`}>
                {/* Header da Coluna */}
                  ...
                  {itensEtapa.length === 0 ? (
                    ...
                    itensEtapa.map((lic) => (
                      <div
                        ...
                        className="bg-card border rounded-lg p-3 shadow-xs hover:shadow-md transition-all cursor-pointer space-y-2 group hover:border-primary/50"
                      >
                        <div className="flex items-start justify-between gap-1">
                          ...
                          <button
                            ...
                            className="text-muted-foreground hover:text-red-600 opacity-0 group-hover:opacity-100 transition-opacity p-0.5"
                          >
                            <Trash2 className="w-3 h-3" />
                          </button>
                        </div>

                        <p className="text-[11px] text-muted-foreground truncate">{lic.orgao || "—"}</p>

                        <div className="flex items-center justify-between text-[11px] pt-1 border-t text-muted-foreground">
                          <span>{lic.uf} • {lic.municipio || "—"}</span>
                          <span className="font-bold text-primary">
                            {(Number(lic.valor) || 0).toLocaleString("pt-BR", { style: "currency", currency: "BRL", maximumFractionDigits: 0 })}
                          </span>
                        </div>

                        {/* Seletor rápido de Status no Kanban */}
                        <div className="pt-1.5 border-t" onClick={(e) => e.stopPropagation()}>
                          <select
                            value={lic.status || "interessado"}
                            onChange={(e) => handleMudarStatus(lic.id, e.target.value)}
                            className="w-full text-[11px] px-2 py-1 bg-muted/40 border rounded font-medium focus:outline-none"
                          >
                            {STATUS_OPTIONS.map((s) => (
                              <option key={s.value} value={s.value}>Mover: {s.label}</option>
                            ))}
                          </select>
                        </div>
                      </div>
